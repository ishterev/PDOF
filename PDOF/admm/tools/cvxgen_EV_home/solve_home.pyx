...
cdef class Solver:
    ...
    #          csolve_home.params.A[i] = ...;
    cdef void load_data(self, double [:, :] xold, double [:, :] u, double [:] xmean, params p): 

         pass            
         
         '''#define optimization parameters
         cdef double [:, :] K = np.asarray(xold) - np.asarray(u) - np.asarray(xmean);       # Normalization parameter

         # contrained least square optimization parameters
         cdef double [:, :] dd = p.rho/ (2*p.gamma*p.alpha + p.rho) * np.asarray(K)
        
         #solve with cvxgen
        
         cdef double [:] AEQ = csolve_home.params.Aeq
         AEQ[...] = self.A
        
         csolve_home.params.Aeq= <double [96]> np.asarray(self.A)
         csolve_home.params.beq= <double [1]> np.asarray(self.R) #R.T
         csolve_home.params.lb= <double [96]> (np.asarray(self.d) * p.xmin) # d.T*xmin
         csolve_home.params.ub= <double [96]> (np.asarray(self.d) * p.xmax) # d.T*xmax
         csolve_home.params.d= <double [96]>(np.asarray.dd)'''


    '''cdef double[:] use_solution(self, csolve_home.Vars vars):
    # In this function, use the optimization result.
    #    ... = vars.x[i];
    
         #cdef double[:] mview = vars.x
        
         return np.zeros(96) #np.asarray(m) #np.float64_t '''

    cdef double[:] solve(self, double [:, :] xold, double [:, :] u, double [:] xmean, params p):  
        csolve_home.set_defaults()  # Set basic algorithm parameters.
        csolve_home.setup_indexing()

        self.load_data(xold, u, xmean, p)

        # Solve our problem at high speed!
        cdef long num_iters = csolve_home.solve()
        # Recommended: check work.converged == 1.

        return np.zeros(96) #self.use_solution(csolve_home.vars)
    ...
    cpdef double[:] test(self): 
        ...
        csolve_home.load_default_data()

        # Solve our problem at high speed!
        cdef int num_iters = csolve_home.solve()
        # Recommended: check work.converged == 1.
