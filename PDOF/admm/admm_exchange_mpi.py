...
xi = np.zeros((T,1)) # np.zeros((T, N))
ui = np.zeros((T,1))
zi = np.zeros((T,1))  # np.zeros((T, N))
x_mean = np.zeros((T,1))
ri = np.zeros((T,1))
...
for i in xrange(MAXITER):#50
        ...
        s_norm = np.sqrt(N) * rho * dnrm2(zi - zi_old)
       
        eps_pri = np.sqrt(T*N) * ABSTOL + RELTOL * max(nxstack, dnrm2(zi)) # dnrm2(-1 * zi)
