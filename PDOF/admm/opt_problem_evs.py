...
class OptProblem_ValleyFilling_MinEnergy(OptimizationProblem):
      ...
      def __init__(self, idx, discharge = False): 
          ...
          data = loadEV('minEnergy', self.idx)              
          ...
          for key,val in data.items() :
              ...
              if(key == 'R_min'):
                 self.R_min = data[key][()].T
                 
              if(key == 'R_max'):
                 self.R_max = data[key][()].T
          ...
          self.A=self.A[1:end-1,:]
          
          end = self.R_max.shape[0]
          self.R=self.R_max[end]
          self.R_min=self.R_min[1:end-1]
          self.R_max=self.R_max[1:end-1]
